    ...
    if (!(param->rc.bStatRead && param->rc.cuTree && IS_REFERENCED(curFrame)))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (param->rc.aqMode == X265_AQ_AUTO_VARIANCE || param->rc.aqMode == X265_AQ_AUTO_VARIANCE_BIASED || param->rc.aqMode == X265_AQ_EDGE || param->rc.aqMode == X265_AQ_EDGE_BIASED || param->rc.aqMode == X265_AQ_VARIANCE_BIASED || param->rc.aqMode == X265_AQ_VARIANCE_AUTO_MIN || param->rc.aqMode == X265_AQ_VARIANCE_AUTO_MIN_BIASED)
                {
                ...
                for (int blockY = 0; blockY < maxRow; blockY += loopIncr)
                {
                    for (int blockX = 0; blockX < maxCol; blockX += loopIncr)
                    {
                        ...
                        if (param->bHDR10Opt)
                        {
                            ...
                            lumaAvg = param->internalBitDepth == 10 ? lumaAvg : param->internalBitDepth == 8 ? lumaAvg << 2 : lumaAvg >> 2;
                            if (lumaAvg < 301)
                                qp_adj += 3;
